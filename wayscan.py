...
class MainWindow(Qtw.QMainWindow, main.Ui_MainWindow):
    ...
    def setup_hardware(self):
        """Configures camera and turntable details
    ...
    def connect_ui(self):
        """Connects UI elements with applicable callbacks and values
    ...
    def refresh_camera_settings(self):
        """Updates camera settings with current camera values
    ...
    def display_config(self):
        ...
        Display configuration dialog where user can update application
        and camera settings
    ...
    def display_ftp(self):
        ...
        Display the FTP dialog where user can copy and transfer
        image data to Wayfair FTP servers
    ...
    def close(self):
        """Action to perform when exiting application
...
class ImageAssociation():
    """Class to contain the image mapping details

    Attributes:
        camera_number (int): The camera number that took the image
        scan_name (string): Name of the scan to associate image with
        series (int): The series number to associate the image with
        image_type (string): The type of image
            (Initiazation, scan, colorcard, background)
        aperture (string): F-Stop used to capture the image
        ISO (int): The ISO used to capture the image
        shutter_speed (string): Shutter speed used to capture the image
        file_path (string): Name of the image file
        dir (string): Directory on the camera where the image file is located
...
class ScanDetails():
    ...
    Attributes:
        num_series (int): The number of series/orientations taken for the scan
        scan_type (string): Type of scan, 'Production', 'Experiment', etc.
        object_type (string): Type of object being scanned, 'Sku', 'Prop', etc.
        notes (string): Any additional notes of interest about the scan
        scan_name (string): Name/ID of the scan
        generate_3d_model (bool): Whether or not we should be trying to
            generate a 3D model or just silos from the scan images
...
def get_pixmap(name):
    """Get the QPixmap represenation of an image

    Args:
        name (string): Name of the image file that we want a pixmap of
